...
//
// Registered ABOVE getMessaging() on purpose: the SDK installs its own 'notificationclick'
// listener from inside that call, and that listener calls stopImmediatePropagation() on any
// notification it recognises as its own. Since every notification we show now comes from the
// SDK (see the onBackgroundMessage comment below), registering after it would mean this
// listener never runs and a tap did nothing but dismiss.
...
if (FIREBASE_WEB_CONFIG.apiKey) {
  ...
  // Deliberately displays NOTHING. The SDK's own push handler already calls
  // registration.showNotification() itself for any payload carrying a `notification` block —
  // unconditionally, and *before* it invokes this hook (see @firebase/messaging's onPush).
  // Ours is not an override of that, it's an addition, so showing one here put two identical
  // notifications in the tray for every order: the SDK's (no icon, so Chrome drew its grey
  // letter avatar) and ours (the brand icon). The title/body/icon of the one remaining
  // notification are set server-side instead, in PushNotificationService's Webpush block.
